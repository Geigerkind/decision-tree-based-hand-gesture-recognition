...
float buffer_cocd_float_x[FRAME_BUFFER_SIZE];
float buffer_cocd_float_y[FRAME_BUFFER_SIZE];
short buffer_cocd_int_x[FRAME_BUFFER_SIZE];
short buffer_cocd_int_y[FRAME_BUFFER_SIZE];

// Arg buffer for the decision tree
float dt_args_float[10];
short dt_args_int[10];
...
void removeFirstFrameFromBuffer() {
  for (int i = 1; i < noFramesInBuffer; i++) {
    buffer_cocd_float_x[i - 1] = buffer_cocd_float_x[i];
    buffer_cocd_float_y[i - 1] = buffer_cocd_float_y[i];
    buffer_cocd_int_x[i - 1] = buffer_cocd_int_x[i];
    buffer_cocd_int_y[i - 1] = buffer_cocd_int_y[i];
  }
  noFramesInBuffer--; 
}
...
void loop() {
  ...
  buffer_cocd_int_x[noFramesInBuffer] = current_frame_buffer[0] + current_frame_buffer[3] + current_frame_buffer[6] - current_frame_buffer[2] - current_frame_buffer[5] - current_frame_buffer[8];
  buffer_cocd_int_y[noFramesInBuffer] = current_frame_buffer[0] + current_frame_buffer[1] + current_frame_buffer[2] - current_frame_buffer[6] - current_frame_buffer[7] - current_frame_buffer[8];

  if (cocd_total > 0) {
    buffer_cocd_float_x[noFramesInBuffer] = (float)(buffer_cocd_int_x[noFramesInBuffer]) / ((float)cocd_total);
    buffer_cocd_float_y[noFramesInBuffer] = (float)(buffer_cocd_int_y[noFramesInBuffer]) / ((float)cocd_total);
  } else {
    buffer_cocd_float_x[noFramesInBuffer] = 0;
    buffer_cocd_float_y[noFramesInBuffer] = 0;
  }
  ...
  if (state == READ_INITIAL_FRAMES) {  // Read first frames that will precide the frames with lower brightness. 
    ...
    if (noFramesToBeRecordedInRecordEndingFrames <= 0) { // If all frames have been read ... 
      ...
      center_of_gravity_distribution_float(buffer_cocd_float_x, noFramesInBuffer, dt_args_float);
      center_of_gravity_distribution_float(buffer_cocd_float_y, noFramesInBuffer, dt_args_float + 5);
      center_of_gravity_distribution_int(buffer_cocd_int_x, noFramesInBuffer, dt_args_int);
      center_of_gravity_distribution_int(buffer_cocd_int_y, noFramesInBuffer, dt_args_int + 5);
      unsigned long featureEndTime = micros();

      unsigned long treeStartTime = micros();
      unsigned char prediction = evaluate_forest(dt_args_float, dt_args_int);
