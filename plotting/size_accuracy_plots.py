...
for feature_set in range(2):
    for set_fraction in set_fractions:
        for ensamble_technique in range(5):
            current_data = data_size
            df = pd.DataFrame().query("ensamble_technique == " + str(ensamble_technique) + " and feature_set == " + str(
                feature_set) + " and set_fraction = " + set_fraction)
            ...
            plt.savefig('./plotting/size_accuracy_plots/feature_set_' + str(feature_set) + '/ensamble_' + str(
                ensamble_technique) + '/tree_per_depth_' + set_fraction + '.png', format='png', dpi=100, pad_inches=0.2,
                        bbox_inches="tight")
            plt.close('all')
            ...
            for max_depth in range(31)[1:]:
                ...
                plt.savefig('./plotting/size_accuracy_plots/feature_set_' + str(feature_set) + '/ensamble_' + str(
                    ensamble_technique) + '/forest_' + str(max_depth) + '_per_size_' + set_fraction + '.png',
